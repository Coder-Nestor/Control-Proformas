                    ...
                    <?php foreach ($ordenes as $ocIndex => $oc): ?>
                        ...
                        <tr class="<?= $rowClass ?>">
                            <td class="text-center">
                                <input type="checkbox" class="form-check-input check-oc"
                                       data-id="<?= (int) $oc['id'] ?>"
                                       data-oce="<?= e($oc['n_oce_interna'] ?? '—') ?>"
                                       data-proforma="<?= e($oc['n_proforma'] ?: '#' . $oc['proforma_id']) ?>"
                                       data-proveedor="<?= e($oc['proveedor_nombre'] ?? '—') ?>"
                                       data-valor="<?= e(fmt_money($oc['valor_proforma'])) ?>"
                                       data-fecha="<?= e(fmt_date($oc['fecha_envio_oce'])) ?>"
                                       data-estado="<?= e($estados[$oc['estado']] ?? $oc['estado']) ?>">
                            </td>
                            ...
                            <td class="text-end pe-3 py-2">
                                <div class="d-flex justify-content-end gap-1">
                                    ...
                                    <?php if (Auth::can('ordenes.eliminar')): ?>
                                        <form method="POST" action="<?= base_url('/ordenes/' . $oc['id'] . '/eliminar') ?>" class="d-inline" onsubmit="return confirm('¿Eliminar esta orden de compra? La factura y entrega asociadas (si existen) también se eliminarán. Esta acción no se puede deshacer.');">
                                            <?= csrf_field() ?>
...
<!-- importar cuántas órdenes de compra se marquen.               -->
<!-- ============================================================ -->
<div class="d-none d-print-block" id="areaImpresionOrdenes"></div>
...
<style>
    @media print {
        ...
        * {
        ...
        .sidebar-backdrop,
        .alert,
        .container-fluid > *:not(#areaImpresionOrdenes) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function checks() {
        return Array.from(document.querySelectorAll('.check-oc'));
    }
        ...
        return '' +
            '<div class="hoja-constancia">' +
                ...
                '<table class="const-tabla">' +
                    '<colgroup><col style="width:17%"><col style="width:15%"><col style="width:20%"><col style="width:15%"><col style="width:16%"><col style="width:17%"></colgroup>' +
                    '<thead><tr><th>N° OCE e interna</th><th>N° Proforma</th><th>Proveedor</th><th>Valor</th><th>Fecha de envío</th><th>Estado</th></tr></thead>' +
    ...
    if (btnImprimir) {
        btnImprimir.addEventListener('click', function () {
            ...
            const contenedor = document.getElementById('areaImpresionOrdenes');
            let html = '';
